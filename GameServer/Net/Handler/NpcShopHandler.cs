...
namespace Server.Handler
{
    public static class NpcShopHandler
    {
        public static void Buy_Req(InPacket lea, Client gc)
        {
            ...
            if (gc.Character.Money >= (money * Quantity))
            {
                ...
                if (((Type == (byte)InventoryType.ItemType.Spend3) ||
                     (Type == (byte)InventoryType.ItemType.Other4)) && (finditem != null))
                {
                    // 合併消費物品跟其他物品
                    if ((finditem.Quantity + Quantity) > 100)
                    {
                        int newqu = Quantity - (100 - finditem.Quantity);
                        gc.Character.Items[(InventoryType.ItemType)finditem.Type, finditem.Slot].Quantity =
                            (short)100;
        ...
        public static void Sell_Req(InPacket lea, Client gc)
        {
            ...
            if (ItemID == 8880011 || ItemID == 8880021 || ItemID == 8880031 || ItemID == 8880041 || ItemID == 8880051 ||
                ItemID == 8880061 || ItemID == 8880071 || ItemID == 8880081 || ItemID == 8880091 || ItemID == 8880101)
                Money = 0;
