...
namespace Server.Ghost.Provider
{
    public static class ItemFactory
    {
        ...
        // Weapon information
        // Item number (int), item name (string), occupation (byte), skill (byte), type (short), attack distance (short), attack speed [Speed] (byte), purchase price (int)
        public static Dictionary<int, ItemData> weaponData = new Dictionary<int, ItemData>();

        // clothing information
        // item number (int), item name (string), occupation (byte), skill (byte), purchase price (int)
        ...
        // Seal Box Information
        // item number (int), item name (string), purchase price (int)
        ...
        public static void Initialize()
        {
            ...
            //==============================================================================
            item.ReadBytes(120); // unknown
            //==============================================================================
            // weapon type starts
            int weaponCount = item.ReadInt32(); // number of weapons

            for (int i = 0; i < weaponCount; i++)
            {
                int itemId = item.ReadInt32(); // Item ID
                byte[] itemNameByteArray = item.ReadBytes(62); // Item Name (Byte [])
                ...
                item.ReadInt16();
                byte Speed = item.ReadByte(); // 攻擊速度 [Speed]
                item.ReadInt16();
                item.ReadInt16();
                item.ReadInt16();
                item.ReadInt16();
                ...
                item.ReadBytes(16);
                item.ReadBytes(20);   
                weaponData.Add(itemId,
                    new ItemData(itemId, itemNameString, job, level, attack, MagicAttack, attackRange, Speed, price,
                        Fusion));
            }
            ...
            for (int i = 0; i < topCount; i++)
            {
                ...
                string itemNameString =
                    Encoding.GetEncoding("UTF-16LE").GetString(itemNameByteArray); // 物品名稱 (Byte[] => String)
                item.ReadByte();
            ...
            for (int i = 0; i < clothingCount; i++)
            {
                ...
                item.ReadBytes(3);
                item.ReadBytes(12);
                clothingData.Add(itemId,
                    new ItemData(itemId, itemNameString, job, level, defense, Str, Dex, Vit, Int, price, Fusion));
            }
            ...
            for (int i = 0; i < ringCount; i++)
            {
                ...
                string itemDescriptionString =
                    Encoding.GetEncoding("UTF-16LE").GetString(itemDescriptionByteArray); // 物品敘述 (Byte[] => String)

                ringData.Add(ItemId,
                    new ItemData(ItemId, itemNameString, Job, Level, Str, Dex, Vit, Int, Magic, Avoid, Attack, Defense,
                        Hp, Mp, Refining, Price));
            }
