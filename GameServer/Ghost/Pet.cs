    ...
    public class Pet
    {
        ...
        public void Save()
        {
            ...
            else
            {
                ...
                this.ID = Database.Fetch("Pets", "id",
                    "cid = '{0}' && itemId = '{1}' && decorateId = '{2}' && name = '{3}' && level = '{4}' && hp = '{5}' && mp = '{6}' && exp = '{7}' && type = '{8}' && slot = '{9}'",
                    this.Character.ID, this.ItemID, this.DecorateID, this.Name, this.Level, this.Hp, this.Mp, this.Exp,
                    this.Type, this.Slot);
