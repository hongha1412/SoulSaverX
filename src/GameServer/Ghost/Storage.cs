...
namespace Server.Ghost
{
	public class Storage
	{
		...
		public Storage(int ItemID, int Quantity, int Spirit, byte Level1, byte Level2, byte Level3, byte Level4,
			byte Level5, byte Level6, byte Level7, byte Level8, byte Level9, byte Level10, byte Fusion, byte IsLocked,
			int Icon, int Term, int Type, int Slot, int Money)
		{
			this.ItemID = ItemID;
			this.Quantity = Quantity;
			this.Spirit = Spirit;
		...
		public void Save()
		{
			...
			else
			{
				...
				this.ID = Database.Fetch("Storages", "id",
					"cid = '{0}' && itemID = '{1}' && quantity = '{2}' && spirit = '{3}' && level1 = '{4}' && level2 = '{5}' && level3 = '{6}' && level4 = '{7}' && level5 = '{8}' && level6 = '{9}' && level7 = '{10}' && level8 = '{11}' && level9 = '{12}' && level10 = '{13}' && fusion = '{14}' && isLocked = '{15}' && icon = '{16}' && term = '{17}' && type = '{18}' && slot = '{19}' && money = '{20}'",
					this.Character.ID, this.ItemID, this.Quantity, this.Spirit, this.Level1, this.Level2, this.Level3,
					this.Level4, this.Level5, this.Level6, this.Level7, this.Level8, this.Level9, this.Level10,
					this.Fusion, this.IsLocked, this.Icon, this.Term, this.Type, this.Slot, this.Money);
