	...
	public class Quest
	{
		...
		public void Save()
		{
			...
			else
			{
				...
				this.ID = Database.Fetch("Quests", "id",
					"cid = '{0}' && questID = '{1}' && stage = '{2}' && requireMonster = '{3}' && completeMonster = '{4}' && questState = '{5}'",
					this.Character.ID, this.QuestID, this.QuestStage, this.RequireMonster, this.CompleteMonster,
					this.QuestState);
