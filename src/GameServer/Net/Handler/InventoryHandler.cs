...
namespace Server.Handler
{
	public static class InventoryHandler
	{
		public static void MoveItem_Req(InPacket lea, Client gc)
		{
			...
			if (SourceType != (byte)InventoryType.ItemType.Pet5 && Source.IsLocked == 1 &&
				TargetType == (byte)InventoryType.ItemType.Equip)
			{
				Source.IsLocked = 0;
			}
			...
			if (TargetType == 0x63 && TargetSlot == 0x63)
			{
				...
				{
					InventoryPacket.CharDropItem(All.Client, Map.ObjectID, Source.ItemID, chr.PlayerX,
						(short)(chr.PlayerY - 50), Quantity == 0 ? 1 : Quantity);
				}

				Map.Item.Add(Map.ObjectID,
					new Drop(Map.ObjectID, Source.ItemID, Quantity == 0 ? (short)1 : (short)Quantity));
				Map.ObjectID++;
			...
			else
			{
				if (chr.Items.getItem(TargetType, TargetSlot) == null)
				{
					if (SourceType != 5 && TargetType != 5)
					{
						...
						if (TargetType == (byte)InventoryType.ItemType.Equip ||
							SourceType == (byte)InventoryType.ItemType.Equip)
				...
				else
				{
					...
					{
						// 合併消費物品跟其他物品
						if (chr.Items[(InventoryType.ItemType)Target.Type, Target.Slot].Quantity + Source.Quantity >
							100)
						{
							short newqu = (short)(Source.Quantity -
												   (100 - chr.Items[(InventoryType.ItemType)Target.Type, Target.Slot]
														.Quantity));
					...
					if (SourceType == (byte)InventoryType.ItemType.Equip &&
						TargetType == (byte)InventoryType.ItemType.Equip1)
					{
						UpdateCharacterInventoryStatus(gc, Source, false);
						UpdateCharacterInventoryStatus(gc, Target, true);
					}
					else if (SourceType == (byte)InventoryType.ItemType.Equip ||
							 TargetType == (byte)InventoryType.ItemType.Equip)
					{
		...
		public static void UseSpend_Req(InPacket lea, Client gc)
		{
			...
			switch (Item.ItemID)
			{
				...
				case 8890011: // 蠟燭
				case 8890021: // 火把
					InventoryPacket.UseSpendStart(gc, chr, chr.PlayerX, chr.PlayerY, Item.ItemID,
						(byte)InventoryType.ItemType.Spend3, Slot);
					break;
				case 8899017: // 逮巴符
					chr.MapX = 27;
					chr.MapY = 1;
		...
		public static void InvenUseSpendStart_Req(InPacket lea, Client c)
		{
			...
			foreach (Character All in map.Characters)
				InventoryPacket.UseSpendStart(All.Client, chr, PositionX, PositionY, Item.ItemID,
					(byte)InventoryType.ItemType.Spend3, Slot);
		...
		public static void PickupItem(InPacket lea, Client gc)
		{
			...
			{
				// 合併消費物品跟其他物品
				chr.Items[(InventoryType.ItemType)finditem.Type, finditem.Slot].Quantity +=
					Map.getDropByOriginalID(OriginalID).Quantity;
			}
			else
			{
