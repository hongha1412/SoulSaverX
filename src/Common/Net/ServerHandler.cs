		...
		: NetworkConnector<TReceiveOP, TSendOP, TCryptograph>, IDisposable where TCryptograph : Cryptograph, new()
	{
		...
		public void Loop()
		{
			while (this.IsAlive && this.IsServerAlive)
			{
				...
				this.Socket.BeginReceive(buffer.Array, buffer.Position, buffer.Capacity, SocketFlags.None,
					new AsyncCallback(this.OnReceive), buffer);
				...
				try
				{
					...
					else
					{
						...
						if (Enum.IsDefined(typeof(TReceiveOP), inPacket.OperationCode))
						{
							...
							{
								case LogLevel.Name:
									Log.Inform("Received {0} packet.",
										Enum.GetName(typeof(TReceiveOP), inPacket.OperationCode));
									break;

								case LogLevel.Full:
									Log.Hex("Received {0} packet: ", inPacket.Array,
										Enum.GetName(typeof(TReceiveOP), inPacket.OperationCode));
									break;
							}
						}
		...
		public void Send(OutPacket outPacket)
		{
			...
			if (Enum.IsDefined(typeof(TSendOP), outPacket.OperationCode))
			{
				switch (PacketBase.LogLevel)
				{
					...
					case LogLevel.Full:
						Log.Hex("Sent {0} packet: ", outPacket.Content,
							Enum.GetName(typeof(TSendOP), outPacket.OperationCode));
						break;
				}
			}
			else
