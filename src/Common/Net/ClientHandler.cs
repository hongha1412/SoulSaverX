		...
		: NetworkConnector<TReceiveOP, TSendOP, TCryptograph>, IDisposable where TCryptograph : Cryptograph, new()
	{
		...
		public ClientHandler(Socket socket, string title = "Client", params object[] args)
		{
			...
			while (this.IsAlive && this.IsServerAlive)
			{
				...
				try
				{
					this.Socket.BeginReceive(this.ReceivalBuffer.Array, this.ReceivalBuffer.Limit,
						this.ReceivalBuffer.Capacity - this.ReceivalBuffer.Limit, SocketFlags.None,
			...
			lock (this)
			{
				if (this.IsAlive)
				{
					...
					if (Enum.IsDefined(typeof(TSendOP), outPacket.OperationCode))
					{
						...
						{
							case LogLevel.Name:
								Log.Inform("Sent {0} packet to {1}.",
									Enum.GetName(typeof(TSendOP), outPacket.OperationCode), this.Title);
								break;

							case LogLevel.Full:
								Log.Hex("Sent {0} packet to {1}: ", outPacket.Content,
									Enum.GetName(typeof(TSendOP), outPacket.OperationCode), this.Title);
								break;
						}
					}
					...
					{
						Log.Hex("Sent unknown (0x{0:X2}) packet to {1}: ", outPacket.Content, outPacket.OperationCode,
							this.Title);
					}
				}
				else
