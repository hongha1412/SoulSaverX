...
namespace Server.Common.Data
{
	...
	{
		public static Dictionary<string, Dictionary<string, Column>> Tables =
		...
		public static void Initialize(bool mcdb)
		{
			...
			foreach (dynamic datum in new Datums("COLUMNS").Populate("TABLE_SCHEMA = '{0}'{1}", Database.DefaultSchema,
				mcdb ? " OR TABLE_SCHEMA = 'mcdb'" : string.Empty))
			{
				Meta.Add(datum);
			}
		}
