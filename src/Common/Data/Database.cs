...
namespace Server.Common.Data
{
	...
	public static class Database
	{
		...
		internal static string CorrectFields(string fields)
		{
			...
			string[] tokens = fields.Replace(",", " ").Replace(";", " ")
		...
		public static bool Exists(string table, string constraints, params object[] args)
		{
			using (MySqlDataReader reader = MySqlHelper.ExecuteReader(Database.ConnectionString,
				string.Format("SELECT * FROM {0} WHERE {1}", table, string.Format(constraints, args))))
			{
		...
		public static void ExecuteScript(string host, string username, string password, string query,
			params object[] args)
		{
			using (MySqlConnection connection =
				new MySqlConnection(string.Format("SERVER={0}; UID={1}; PASSWORD={2};", host, username, password)))
			{
				connection.Open();
		...
		public static TemporaryConnection TemporaryConnection(string host, string schema, string username,
			string password)
		{
