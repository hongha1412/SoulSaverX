...
namespace Server.Common.Data
{
	public class Datums : IEnumerable<Datum>
	{
		...
		internal void PopulateInternal(string fields, string constraints)
		{
			...
			string query = string.Format("SELECT {0} FROM {1}{2}",
				fields == null ? "*" : Database.CorrectFields(fields), this.Table,
				constraints != null ? " WHERE " + constraints : string.Empty);
	...
	public class Datum : DynamicObject
	{
		...
		public dynamic PopulateWith(string fields, string constraints, params object[] args)
		{
			this.Populate(string.Format("SELECT {0} FROM {1} WHERE {2}", Database.CorrectFields(fields), this.Table,
				string.Format(constraints, args)));
		...
		public void Insert()
		{
			...
			foreach (KeyValuePair<string, object> loopPair in this.Dictionary)
			{
				fields += string.Format("'{0}'",
					loopPair.Value is bool ? (((bool)loopPair.Value) ? 1 : 0) : loopPair.Value);
				processed++;
		...
		public void Update(string constraints, params object[] args)
		{
			...
			foreach (KeyValuePair<string, object> loopPair in this.Dictionary)
			{
				fields += string.Format("{0}='{1}'", loopPair.Key,
					loopPair.Value is bool ? (((bool)loopPair.Value) ? 1 : 0) : loopPair.Value);
				processed++;
