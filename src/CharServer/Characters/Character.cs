...
namespace Server.Characters
{
	public sealed class Character
	{
		...
		public void Load(bool IsFullLoad = true)
		{
			...
			this.Job = (byte)datum.job;
			this.created_at = (string)datum.created_at;
			this.deleted_at = (string)datum.deleted_at;
			this.Items.Load();
			this.Storages.Load();
			this.Skills.Load();
			this.Keymap.Load();
		...
		public void Save()
		{
			...
			datum.job = this.Job;
			datum.created_at = this.created_at;
			datum.deleted_at = this.deleted_at;
			if (this.Assigned)
			{
			...
			Log.Inform("The data for the char '{0}' has been saved to the database.", this.Name);
		}
