...
{
	public sealed class
		InteroperabilityClient : ServerHandler<InteroperabilityMessage, InteroperabilityMessage, BlankCryptograph>
	{
		public InteroperabilityClient(IPEndPoint remoteEP, string code) : base(remoteEP, "Login server",
			new object[] { code })
		{
		}
		...
		{
			get { return MessageServer.IsAlive; }
		}
				...
				case ServerUtilities.ServerRegistrationResponse.Valid:
					{
						...
						Log.Success("Registered Message as {0} at {1}.",
							ServerUtilities.WorldNameResolver.GetName(MessageServer.WorldID), MessageServer.RemoteEndPoint);
					}
					break;
