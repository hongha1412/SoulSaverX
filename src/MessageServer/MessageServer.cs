	...
	internal static class MessageServer
	{
		...
		start:
			...
			try
			{
				...
				MessageServer.RemoteEndPoint = new IPEndPoint(Settings.GetIPAddress("ExternalIP", "Message"),
					Settings.GetInt("Port", "Message"));
		...
		private static void OnAcceptSocket(IAsyncResult ar)
		{
			...
			{
			} // NOTE: Seems to be an issue with the ManualResetEvent or the TcpListener, most likely to be a disposal issue.
		}
